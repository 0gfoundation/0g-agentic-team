...
1. **Two kinds of issues**: created by the owner = task card (must contain mission + acceptance criteria; the owner creating it personally means the confirmation gate is passed); created by a member = proposal or bug report (the member drafts and signs the body, the lead relays it verbatim) — proposals also pass the confirmation gate: the lead annotates its assessment in the issue, and only ones the owner nods on (comment/emoji) become tasks. Members cannot initiate tasks for themselves, and **the lead never files un-instructed issues** — it has a standing owner session; a lead idea is raised there first and opens as an issue only once instructed. **A lead-posted issue always requires prior explicit owner instruction, and its body carries an inline proof block** (§5.2 — sign the body with the proof block stripped) plus a line recording the authorization (`authorized-by-owner: <time/channel>`). The instruction itself lives in the owner↔lead session; an owner-instructed issue needs no in-issue owner confirmation — the in-issue nod (comment/emoji) is only for *proposals*, which the owner did not instruct.
2. **Claim receipt + minimum staffing**: a member's first comment on an issue is the claim receipt (`claim: <role label> @ <time>, expected <deliverable>`), followed by a lead confirmation comment (`lead confirmed claim`) — prevents racing/duplicate work. **Every issue is staffed by at least 2 agents: one coder and one reviewer (more allowed), both settled at claim time** — an issue does not enter development until a reviewer is identified (exception: review tasks, rule 7). No work starts before the claim is confirmed.
...
4. **Done ≠ merged**: issues are closed by PR merges (`Closes #N`), never manually — every issue's closure passes through a final-review gate (member approve + lead final review + owner merge decision). Exception: review tasks, rule 7.
5. **Muster issue = onboarding**: joining happens in a muster issue, opened by the owner directly or by the lead after explicit owner instruction (rule 1 applies: proof-carrying body + recorded authorization; template: `docs/seeds/muster-issue.md`). Each new member checks in with a structured comment (the member drafts and signs it itself; the lead relays the bytes verbatim):
...
7. **Review task (single-reviewer mode)**: an issue whose deliverable is a review of a target PR, not code. Staffed by ≥1 reviewer — rule 2's coder+reviewer pairing applies only to issues that deliver code. The deliverable is a proof-carrying review posted on the target PR, its `context:` naming the target `<org>/<repo>#<PR>` (pinning the review to that PR; it cannot be replayed elsewhere). Exempt from rule 4: with no PR of its own, the issue is closed after the lead posts a proof-carrying acceptance comment (review delivered, signature and context verified) and the owner closes it.
