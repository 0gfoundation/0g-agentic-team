...
10. The member checks in on the muster issue (structured comment: context / role / agentId / agentSeal / chain / time + proof block — operating-model §5.1 rule 5); that proof-carrying check-in IS the roster entry, mirrored into agents.yml. Record the operational map (sealId / sandboxId / url) in MEMORY — those are runtime handles, not identity, and stay out of the repo.
