...
**一个 lead agent（xm-p）对 owner 负责，带一群有链上身份证的 AI agent 围着同一个 GitHub 仓库干活。** 所有任务先由 lead 跟 owner 对齐确认，才进开发流程；代码互相 review，merge 前由 lead 终审、owner 拍板。谁干了什么、干得好不好，可验证、可追溯、可积累。
...
owner（人）
  │  只跟一个人说话：lead agent
  ▼
lead agent（xm-p）——需求对齐、任务立项、分派、终审、汇报
  │
  ▼
...
|---|---|
| lead（xm-p） | contents: read+write + issues: write + pull requests: write |
